...
JOIN Events ON Players.player_id = Events.player_id
JOIN Shots ON Events.event_id = Shots.event_id
JOIN Matches ON Matches.match_id = Events.match_id AND Matches.season_id IN (90, 42, 4) AND Shots.first_time = TRUE AND Matches.competition_id = 11
GROUP BY Players.player_id
...
JOIN Events ON Players.player_id = Events.player_id
JOIN Passes ON Events.event_id = Passes.event_id
JOIN Matches ON Events.match_id = Matches.match_id AND Matches.season_id = 90 AND Passes.through_ball = TRUE AND Matches.competition_id = 11
GROUP BY Players.player_id
HAVING COUNT(Passes.event_id) > 0
...
AND Matches.season_id = 90 AND Passes.through_ball = TRUE AND Matches.competition_id = 11
GROUP BY Teams.team_id
HAVING COUNT(Passes.event_id) > 0
ORDER BY num_through_balls DESC; """
...
JOIN Events ON Players.player_id = Events.player_id
JOIN Dribbles ON Events.event_id = Dribbles.event_id
JOIN Matches ON Events.match_id = Matches.match_id AND Matches.season_id IN (90, 42, 4) AND Dribbles.outcome = 'Complete' AND Matches.competition_id = 11
GROUP BY Players.player_id
HAVING COUNT(Players.player_id) > 0
