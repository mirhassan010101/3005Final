...
def Q_1(conn, execution_time):
    ...
    query = """ SELECT Players.player_name, AVG(Shots.xg_score) AS average_xg_score FROM Players
JOIN Events ON Events.player_id = Players.player_id
JOIN Shots ON Events.event_id = Shots.event_id
JOIN Matches ON Matches.match_id = Events.match_id AND Matches.season_id = 90 AND Matches.competition_id = 11
...
def Q_2(conn, execution_time):
    ...
    query = """ SELECT Players.player_name, COUNT(Shots.event_id) AS num_shots FROM Players
...
def Q_3(conn, execution_time):
    ...
    query = """ SELECT Players.player_name, COUNT(Shots.event_id) AS num_first_shots FROM Players
JOIN Events ON Players.player_id = Events.player_id
JOIN Shots ON Events.event_id = Shots.event_id AND Shots.first_time = TRUE
JOIN Matches ON Matches.match_id = Events.match_id AND Matches.season_id IN (90, 42, 4) AND Matches.competition_id = 11
GROUP BY Players.player_id
HAVING COUNT(Players.player_id) > 0
ORDER BY num_first_shots DESC;"""
...
def Q_4(conn, execution_time):
    ...
    query = """ SELECT Teams.team_name, COUNT(Passes.event_id) AS num_passes FROM Teams
JOIN Events ON Teams.team_id = Events.team_id
...
def Q_5(conn, execution_time):
    ...
    query = """SELECT Players.player_name, COUNT(Passes.event_id) AS num_intended_recipient FROM Players
JOIN Passes ON Players.player_id = Passes.recipient_id
JOIN Events ON Events.event_id = Passes.event_id
JOIN Matches ON Matches.match_id = Events.match_id AND  Matches.season_id = 44 AND Matches.competition_id = 2
GROUP BY Players.player_id
HAVING COUNT(Passes.event_id) > 0
...
def Q_6(conn, execution_time):
    ...
    query = """SELECT Teams.team_name,COUNT(Shots.event_id) AS num_shots FROM Teams
JOIN Events ON Teams.team_id = Events.team_id
JOIN Shots ON Events.event_id = Shots.event_id
JOIN Matches ON Events.match_id = Matches.match_id AND Matches.season_id = 44 AND Matches.competition_id = 2
GROUP BY Teams.team_id
HAVING COUNT(Shots.event_id) > 0
ORDER BY num_shots DESC; """
...
def Q_7(conn, execution_time):
    ...
    query = """SELECT Players.player_name, COUNT(Passes.event_id) AS num_through_balls FROM Players
JOIN Events ON Players.player_id = Events.player_id
JOIN Passes ON Events.event_id = Passes.event_id AND Passes.through_ball = TRUE
JOIN Matches ON Events.match_id = Matches.match_id AND Matches.season_id = 90 AND Matches.competition_id = 11
GROUP BY Players.player_id
HAVING COUNT(Passes.event_id) > 0
...
def Q_8(conn, execution_time):
    ...
    query = """SELECT Teams.team_name, COUNT(Passes.event_id) AS num_through_balls FROM Passes
JOIN Events ON Passes.event_id = Events.event_id AND Passes.through_ball = TRUE
JOIN Matches ON Events.match_id = Matches.match_id AND Matches.competition_id = 11 AND Matches.season_id = 90
JOIN Teams ON Events.team_id = Teams.team_id
GROUP BY Teams.team_id
HAVING COUNT(Passes.event_id) > 0
ORDER BY num_through_balls DESC;"""
...
def Q_9(conn, execution_time):
    ...
    query = """SELECT Players.player_name, COUNT(Players.player_name) AS num_successful_dribbles FROM Players
JOIN Events ON Players.player_id = Events.player_id
JOIN Dribbles ON Events.event_id = Dribbles.event_id AND Dribbles.outcome = 'Complete'
JOIN Matches ON Events.match_id = Matches.match_id AND Matches.season_id IN (90, 42, 4) AND Matches.competition_id = 11
GROUP BY Players.player_id
HAVING COUNT(Players.player_id) > 0
...
def Q_10(conn, execution_time):
    ...
    query = """ SELECT Players.player_name, COUNT(Dribbled_Past.event_id) AS num_dribbles FROM Players
JOIN Events ON Players.player_id = Events.player_id
JOIN Dribbled_Past ON Events.event_id = Dribbled_Past.event_id
JOIN Matches ON Events.match_id = Matches.match_id AND Matches.season_id = 90 AND Matches.competition_id = 11
GROUP BY Players.player_id
HAVING COUNT(Dribbled_Past.event_id) > 0
ORDER BY num_dribbles ASC;"""
